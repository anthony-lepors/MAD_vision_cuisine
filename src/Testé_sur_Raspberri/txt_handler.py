...
jsonPo={"nom":"Plaque de cuisson","nature":"Plaque","dimensions":{"QR/droite/gauche":-126.0,"QR/haut/bas":237.5,"Longueur":510,"Largeur":287,"Hauteur":7,"Unité":"mm"},"cercles":[{"PosX":-1.5,"PosY":125,"Rayon":100.0,"Unité":"mm"},{"PosX":1,"PosY":-77.5,"Rayon":82.5,"Unité":"mm"}]}
Library["Library"].append(jsonPo)

jsonPo={"nom":"Plaque de decoupe","nature":"Plaque","dimensions":{"QR/droite/gauche":-120.0,"QR/haut/bas":129.0,"Longueur":367,"Largeur":277,"Hauteur":10,"Unité":"mm"},"cercles":[]}
Library["Library"].append(jsonPo)

jsonPo={"nom":"poele 1 personne","nature":"poêle","dimensions":{"QR/Centre":135,"QR/Poignée":145,"LargeurPoignée":30,"HautPoignée":15,"BasPoignée":35,"rotationPoignée":5,"RayonCentre":80,"RayonBords":80,"Hauteur":90,"Unité":"mm"}}
Library["Library"].append(jsonPo)

jsonPo={"nom":"Wok 1 personne","nature":"poêle","dimensions":{"QR/Centre":152.5,"QR/Poignée":150,"LargeurPoignée":30,"HautPoignée":20,"BasPoignée":45,"rotationPoignée":0,"RayonCentre":92.5,"RayonBords":122.5,"Hauteur":65,"Unité":"mm"}}
Library["Library"].append(jsonPo)
...
jsonPo={"nom":"Curseur","nature":"Curseur","dimensions":{"QR/droite/gauche":0.0,"QR/haut/bas":0.0,"LongueurPoignée":15,"LargeurPoignée":10,"RayonBords1":15,"RayonBords2":15,"Hauteur":15,"HauteurCurseur":5,"Unité":"mm"}}
Library["Library"].append(jsonPo)
...
for z in Library["Library"]:
    ...
    if (nature=="poêle"):
        ...
        Hp=int(z["dimensions"]["HautPoignée"])
        Bp=int(z["dimensions"]["BasPoignée"])
        ...
        for k in [Bp,Hp]:
            for i in [-LP/2,+LP/2]:
                test["Dimensions"].append({"x":int(i),"y":j,"z":k,"w":1})
        for i in range(n):
            test["Dimensions"].append({"x":int(np.cos(alpha*i)*RB),"y":int(centre_poeleY+np.sin(alpha*i)*RB)*np.cos(RP),"z":-np.sin(RP)*(centre_poeleY+np.sin(alpha*i)*RB),"w":1})
            test["Dimensions"].append({"x":int(np.cos(alpha*i)*RC),"y":int(centre_poeleY+np.sin(alpha*i)*RC)*np.cos(RP),"z":H-np.sin(RP)*(centre_poeleY+np.sin(alpha*i)*RC),"w":1})
    ...
    if (nature=="Curseur"):
        ...
        LoP=int(z["dimensions"]["LongueurPoignée"])
