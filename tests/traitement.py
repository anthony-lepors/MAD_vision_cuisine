...
class traitement:
    ...
    #fonction de lecture du fichier JSON (adapté pour la lecture de la librairie des dimensions, actuellement déprécié)
    def readQR(self,QR):
        try:
            json.loads(QR)
        ...
        readJson=json.loads(QR)
        nom=(readJson["nom"])
        nature=(readJson["nature"])
        if (nature=="poêle"):
            QC=int(readJson["dimensions"]["QR/Centre"])
            QP=int(readJson["dimensions"]["QR/Poignée"])
            LP=int(readJson["dimensions"]["LargeurPoignée"])
            RC=int(readJson["dimensions"]["RayonCentre"])
            RB=int(readJson["dimensions"]["RayonBords"])
            H=int(readJson["dimensions"]["Hauteur de poêle"])
            unit=(readJson["dimensions"]["Unité"])
            centre_poeleY=-QC
            points=[]
            ...
            for k in [0,H]:
                for j in [QP,-QC+RB]:
                    for i in [-LP/2,+LP/2]:
                        points.append([int(i),j,k,1])
            for i in range(self.n):
                points.append([int(np.cos(alpha*i)*RC),int(centre_poeleY+np.sin(alpha*i)*RC),H,1])
                points.append([int(np.cos(alpha*i)*RB),int(centre_poeleY+np.sin(alpha*i)*RB),0,1])
            points.append([0,int(centre_poeleY),H,1])
            self.Points=points
            return True
        else:
            return False
    #déprécié à cause de Displace
    def load_coordinate(self,rx=0,ry=0,rz=0,s=1,tx=0,ty=0,tz=0):
        ...
        self.transform=np.matmul(transform,translation)
            
    #déprécié à cause de Displace
    def GetPos(self):
        Points=np.matmul(self.Points,self.transform)
